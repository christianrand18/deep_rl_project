...
episode length (`num_days`). Also includes `nometa` baselines
(`--meta_policy none`, no HRL) at both brand momentum strengths and episode
lengths for comparison against the meta-policy runs.
...
| Seed | 10 |
| Meta-policy | one (agent 0), or none for `nometa` baselines |
| Episodes | 100k |
...
| run_bmg1_nometa_d7_s10 | 1 | none (nometa) | – | 7 | 7 | 8 |
| run_bmg1_nometa_d28_s10 | 1 | none (nometa) | – | 28 | 28 | 32 |
| run_bmg5_nometa_d7_s10 | 5 | none (nometa) | – | 7 | 7 | 8 |
| run_bmg5_nometa_d28_s10 | 5 | none (nometa) | – | 28 | 28 | 32 |

`multiplier` runs omit `--meta_action_mode`/`--meta_reg_lambda` (defaults).
`nometa` runs additionally omit `--meta_agent` (no meta-policy is active).
Picard strategy/tolerance/Anderson params are left at their defaults
(anderson, m=5, tol=5e-3, max_iters=10). Total CPU request across the group: 320.
