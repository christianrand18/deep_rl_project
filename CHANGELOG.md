...
- Add Picard fixed-point iteration solver for multi-day episodes (`--parallel_days`) — treats the N-day chain as a fixed-point system with pre-sampled seeds and meta-policy noise held constant per episode. Default strategy (Tier 1) back-calculates observed market shares from the EMA formula and re-propagates the full BM trajectory analytically in one sequential pass, converging in K≈2 iterations on warm-started episodes vs. N+1 for pure Jacobi. Supports damped updates via `--picard_omega` for high bm_gamma settings.
