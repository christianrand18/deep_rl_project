                ...
                if env.mode == 0:
                    ...
                    for a in [0, 1]:
                        if a == fix_agent:
                            ...
                            if env.od_price_actions:
                                # Mode 2 OD action shape: [nregion, nregion+1]
...
if not args.test:
    ...
    # Worker pool for parallel day execution. set_worker_state must run
    # *before* make_pool so workers inherit env/model_agents/etc. via fork.
    picard_pool = None
    picard_num_workers = 0
    if args.parallel_days and args.picard_parallel_workers > 1:
        set_worker_state(env=env, model_agents=model_agents, args=args,
                         meta_policies=meta_policies, solveRebFlow=solveRebFlow)
        picard_num_workers = args.picard_parallel_workers
        picard_pool = make_pool(picard_num_workers)
    ...
    for i_episode in epochs:
        ...
        # Workers' weights diverged from main after the previous episode's
        # optimizer step; push the fresh state_dict out before any day work.
        # Idempotent on episode 0 (workers and main agree from fork time).
        if picard_pool is not None:
            broadcast_state_dicts(picard_pool, picard_num_workers, model_agents, args.fix_agent)

        # Final Picard iteration's worker results (parallel path). Overwritten
        # on each rerun loop iteration; only the converged one drives training.
        last_parallel_results = None

        rerun = True
        while rerun:
            # Reset per-iteration tracking vars. For the sequential path this runs once;
            # for Picard it runs K times so each pass starts with clean accumulators.
            ...
            # Picard: clear low-level buffers so only the final iteration's
            # transitions reach training_step().
            if args.parallel_days and args.mode not in [3, 4]:
                for a in [0, 1]:
                    if a != args.fix_agent:
            ...
            ctx = SimpleNamespace(
                ...
                episode_total_operating_cost=episode_total_operating_cost,
                episode_waiting=episode_waiting,
                episode_rejected_demand=episode_rejected_demand,
                episode_total_demand=episode_total_demand,
                episode_rejection_rates=episode_rejection_rates,
                episode_true_profit=episode_true_profit,
                episode_adjusted_profit=episode_adjusted_profit,
                episode_unprofitable_trips=episode_unprofitable_trips,
                actions_price=actions_price,
                actions_effective_price=actions_effective_price,
                meta_shaping_term=meta_shaping_term,
                episode_logprobs=episode_logprobs,
                day_logs=[],
            )
            # Mode-specific concentration trackers attached conditionally so
            # ctx only carries the trackers that exist for the current mode.
            if env.mode == 0:
                ctx.actions_concentration_dirichlet = actions_concentration_dirichlet
                ctx.episode_min_concentration_dirichlet = episode_min_concentration_dirichlet
                ctx.episode_max_concentration_dirichlet = episode_max_concentration_dirichlet
            elif env.mode == 1:
                ctx.actions_concentration_alpha = actions_concentration_alpha
                ctx.actions_concentration_beta = actions_concentration_beta
                ctx.episode_min_concentration_alpha = episode_min_concentration_alpha
                ctx.episode_max_concentration_alpha = episode_max_concentration_alpha
                ctx.episode_min_concentration_beta = episode_min_concentration_beta
                ctx.episode_max_concentration_beta = episode_max_concentration_beta
            else:  # mode 2
                ctx.actions_concentration_alpha = actions_concentration_alpha
                ctx.actions_concentration_beta = actions_concentration_beta
                ctx.actions_concentration_dirichlet = actions_concentration_dirichlet
            ...
            if picard_pool is not None:
                # Parallel: workers run all N days, ship back captures, ctx
                # deltas (merged into ctx in place), per-day grad packets,
                # and day_logs. Only the final iteration's packets survive
                # the rerun-loop overwrite below.
                last_parallel_results = run_days_parallel(ctx, picard_solver, picard_pool)
            else:
                for i_day in range(args.num_days):
                    run_day(i_day, ctx)

            # Drain day_logs queued by run_day (sequential path) or by
            # run_days_parallel's merge (parallel path). Matches the legacy
            # "log every Picard iteration's days" behaviour.
            if not args.test:
                for log in ctx.day_logs:
                    wandb.log(log)
            ctx.day_logs = []

            # Pull reassigned scalars/dicts back into locals so the post-loop
            # code reads unchanged. In-place-mutated containers share identity
            # with ctx fields and don't need rebinding.
            obs = ctx.obs
            ...
            # ── end of for i_day ────────────────────────────────────────────
            if args.parallel_days:
                rerun = picard_solver.next_iteration(env)
                if rerun:
                    obs = env.reset()
            else:
                rerun = False
        ...
        if picard_pool is not None:
            # Parallel path: workers already computed per-day partial gradients
            # on the final Picard iteration. Aggregate them (sum + divide by
            # total transitions = ∇ of sequential mean loss), clip, step.
            grad_norms = apply_aggregated_gradients(
                last_parallel_results, model_agents, args, update_actor
            )
        elif args.mode not in [3, 4]:
            # Sequential path: training_step on each non-fix agent.
            for a in [0, 1]:
                if a == args.fix_agent:
    ...
    # Tear down the parallel worker pool, if any.
    if picard_pool is not None:
        picard_pool.close()
        picard_pool.join()
