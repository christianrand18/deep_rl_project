...
You will also need a **Weights & Biases** account for experiment tracking. Create a free account at [wandb.ai](https://wandb.ai). Run `wandb login` in the terminal to authenticate.
...
Add CPLEX installation path
```bash
export CPLEX_PATH=<path>
```
...
The following directories will be automatically generated by the training scripts
